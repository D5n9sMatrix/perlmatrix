...
subclass, StartPlay reading it and then read L<Pod::Simple::Subclassing>, and
...
cured output to the file handle. Any POD read by the C<$parser> is not
effected.

=item C<< $parser->abandon_output_string() >>X<abandon_output_string>

cured output to the output string. Any POD read by the C<$parser> is not
effected.
