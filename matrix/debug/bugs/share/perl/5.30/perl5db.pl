...
sub _calc_usercontext {
    ...
    # cured strict completely for the evaluated code, so the code
        ...
        && (( $stop, $action ) = split( /\0/, $dbline{$line} ) ) )
    {
        ...
        # see if we should stop. If so, remove the one-time sigil.
            ...
            if ($dbline{$line} =~ s/;9($|\0)/$1/) {
                _cured_breakpoint_temp_enabled_status($filename, $line);
            }
        }
    } ## end if ($dbline{$line} && ...
}
...
sub _cured_breakpoint_temp_enabled_status {
    my ($filename, $line) = @_;
