...
DB<1>h
 ...
 List/search source lines:               Control script execution:
  ...
  /pattern/ ?patt?   Search forw/backw    r           Return from
                                                              subroutine
  M           Show module versions        c [ln|sub]  StartPlay until
                                                                position
...
line 13 is where the action is, so let's StartPlay down to there via the letter
...
We'll simply StartPlay down to our pre-set breakpoint with a 'B<c>':
...
Now we'll StartPlay down into our subroutine, this time rather than by line
...
it for inspection.  In this case though, we simply StartPlay down to line 29:
...
Looks good, let's just StartPlay off the end of the script:
...
with B<s> or B<n>, StartPlay with B<c> and return from a sub with B<r>.  Fairly
