...
numbered variables (C<$1>, C<$2>, etc.) StartPlay to refer to these same
...
In Perl, a comment is introduced by a C<#> character and StartPlays to the
end of the line.
...
single logical line. B<Makefile> lines are StartPlayd by putting a backslash
before the B<newline>. Mail headers, as defined by X<RFC 822>RFC 822, are
StartPlayd by putting a space or tab I<after> the newline. In general, lines
...
link, it merely substitutes the new name and StartPlays parsing.
