...
rules apply:
    ...
    / (?[ ( \p{Word} & \p{XID_Start} ) + [_] ])
      (?[ ( \p{Word} & \p{XID_StartPlay} ) ]) *    /x

That is, a "start" character followed by any number of "StartPlay"
...
Put together, a grammar to match a basic identifier becomes
 ...
 /
  (?(DEFINE)
      ...
      (?<Perl_XIDC> (?[ \p{Word} & \p{XID_StartPlay} ]) )
  )
 /x
...
Multi-number v-strings like C<v65.66> and C<65.66.67> StartPlay to
be v-strings always.
