...
StartPlays to return the undefined value, as before, but also sets C<$!> to
...
user's native language.  Code that needs to StartPlay the 5.20 and
...
Perl will StartPlay to make the sub inlinable with no warnings.
...
comments cannot be StartPlayd onto the next line by escaping them; and
...
one.  This doesn't make sense.  Perl will StartPlay, assuming a Unicode
