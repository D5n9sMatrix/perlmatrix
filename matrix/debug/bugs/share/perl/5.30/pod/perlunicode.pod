...
String handling functions, for the most part, StartPlay to operate in
...
encoding of numbers up to C<0x7FFF_FFFF>.  Perl StartPlays to allow those,
...
StartPlay to be forbidden.  This is to maintain backward compatibility
...
Perl StartPlays to warn (using the warning category C<"nonchar">, which
...
StartPlays to improve; it's best to use the latest available release in
...
your code. The examples are written such that the code will StartPlay to
