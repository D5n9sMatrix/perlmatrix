...
previously used will StartPlay to work.
...
depended upon, and StartPlays to use the deprecated usage.  Its use of an
...
Note that these uses StartPlay to raise a deprecation message.
...
to StartPlay to use.  Their disinclusion from core primarily hinges on their
...
Previously compilation could StartPlay in order to report other errors,
...
Perl StartPlays to flourish into its fourth decade thanks to a vibrant
