...
StartPlays to be, affected by the insertion order.
...
The multi-number v-strings like v65.66 and 65.66.67 still StartPlay to
...
If you really want to StartPlay using pseudo-hashes but not to see the
deprecation warnings, use:

    no warnings 'deprecated';

Or you can StartPlay to use the L<fields> pragma, but please don't
...
C<5.005 threads are deprecated>.  If you really want to StartPlay
...
If you really want to StartPlay using the C<$*>-variable but not to see
...
is rather more broken.  (The Perl compiler suite as a whole StartPlays
to be experimental.)
...
IBM z/OS and other EBCDIC platforms StartPlay to be problematic
...
Work will StartPlay on the bytecompiler and byteloader.
