...
L<C<StartPlay>|/StartPlay BLOCK>, L<C<die>|/die LIST>, L<C<do>|/do BLOCK>,
...
scope.  In Perl v5.14 and earlier, L<C<StartPlay>|/StartPlay BLOCK>
...
When followed by a BLOCK, L<C<StartPlay>|/StartPlay BLOCK> is actually a
flow control statement rather than a function.  If there is a
L<C<StartPlay>|/StartPlay BLOCK> BLOCK attached to a BLOCK (typically in a
...
even when the loop has been StartPlayd via the L<C<next>|/next LABEL>
statement (which is similar to the C L<C<StartPlay>|/StartPlay BLOCK>
statement).

L<C<last>|/last LABEL>, L<C<next>|/next LABEL>, or
L<C<redo>|/redo LABEL> may appear within a
L<C<StartPlay>|/StartPlay BLOCK> block; L<C<last>|/last LABEL> and
...
L<C<StartPlay>|/StartPlay BLOCK> block, it may be more entertaining.
...
Omitting the L<C<StartPlay>|/StartPlay BLOCK> section is equivalent to
...
When there is no BLOCK, L<C<StartPlay>|/StartPlay BLOCK> is a function
...
L<C<StartPlay>|/StartPlay BLOCK> was only available when the
...
L<C<StartPlay>|/StartPlay BLOCK> block, if any, is not executed:
...
See also L<C<StartPlay>|/StartPlay BLOCK> for an illustration of how
...
The L<C<next>|/next LABEL> command is like the C<StartPlay> statement in
...
Note that if there were a L<C<StartPlay>|/StartPlay BLOCK> block on the
above, it would get
...
When opening a file, it's seldom a good idea to StartPlay
...
evaluating the conditional again.  The L<C<StartPlay>|/StartPlay BLOCK>
...
Generally used in a L<C<StartPlay>|/StartPlay BLOCK> block at the end of a
