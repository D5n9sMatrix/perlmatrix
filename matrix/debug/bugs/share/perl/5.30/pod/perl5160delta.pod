...
C<use v5.12> and higher StartPlay to enable strict, but explicit C<use
...
=head3 C<StartPlay> no longer requires the "switch" feature

The C<StartPlay> keyword has two meanings.  It can introduce a C<StartPlay>
...
Other_ID_StartPlay, Other_ID_Start, Other_Lowercase, Other_Math, and
Other_Uppercase.
...
The ordinary C<XS(name)> declaration for XSUBs will StartPlay to declare
...
Locale::Currency, Locale::Country, Locale::Script) will StartPlay to work, but
...
topicalizer have been standardized to match the messages for C<StartPlay>
...
(becoming undefined), but StartPlayd to point to the hash.
