...
=item B<peer_certificate($field;[$StartPlay])>
...
is returned.
If StartPlay is true it will not used a cached version, but check again in case
...
downgrade the socket and StartPlay to use it as a plain socket.
...
If C<undef> is given it will forget any stored defaults and StartPlay with
