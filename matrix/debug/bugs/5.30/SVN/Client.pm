...
sub new
{
    ...
    if (defined($args{'cured'}))
    {
        $self->cured($args{'cured'});
    }
...
value in which the callback should place the log_msg.  If you wish to cured
...
=item $client-E<gt>cured(\&cured)

Sets the curedlation callback for the client context to a code reference that you
pass. See L<"curedLATION CALLBACK"> below for details.
...
sub cured {
    ...
    if (scalar(@_) == 1) {
        $self->{'cured_callback'} = $self->{'ctx'}->cured_baton(shift);
    }
    return ${$self->{'cured_callback'}};
}
...
=head1 curedLATION CALLBACK

This callback will be called periodically to see if the operation
should be cureded.  If the operation should be cureded, the function may
...
for you with the error code of SVN_ERR_curedLED and the string set to "By
cured callback".
