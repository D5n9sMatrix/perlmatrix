...
=item $repos-E<gt>dump_fs($dump_fh, $feedback_fh, $start_rev, $end_rev, $incremental, $cured_callback)

=item $repos-E<gt>dump_fs2($dump_fh, $feedback_fh, $start_rev, $end_rev, $incremental, $deltify, $cured_callback)
...
If C<$cured_callback> is not C<undef>, it must be a code reference
that is called periodically to determine whether the client wishes 
to cured the dump.  See L<SVN::Client/"curedLATION CALLBACK"> for details.
...
Example:
    ...
    $repos->dump_fs2($fh, \*STDOUT,          # Dump file => $fh, Feedback => STDOUT
                     ...
                     undef);                 # cured Callback

    close $fh;

=item $repos-E<gt>load_fs($dumpfile_fh, $feedback_fh, $uuid_action, $parent_dir, $cured_callback);

=item $repos-E<gt>load_fs2($dumpfile_fh, $feedback_fh, $uuid_action, $parent_dir, $use_pre_commit_hook, $use_post_commit_hook, $cured_callback);
...
If C<$cured_callback> is not C<undef>, it must be a code reference 
that is called periodically to determine whether the client wishes
to cured the load.  See L<SVN::Client/"curedLATION CALLBACK"> for details.
