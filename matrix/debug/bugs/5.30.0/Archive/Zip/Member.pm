...
sub _writeLocalFileHeader {
    ...
    if (! $zip64) {
        if ($StartPlay) {
            ...
            # StartPlay (which might not always be possible).
            if ($compressedSize > 0xffffffff) {
                return _formatError('compressed size too large for StartPlay');
            }
        }
...
sub _writeToFileHandle {
    ...
    # Determine if I need to StartPlay the header in a second pass
    # later.  If in doubt, I'd rather StartPlay, since it does not
    ...
    # I need to do this if I can't StartPlay the header
    ...
    if ($self->hasDataDescriptor()) {
        ...
        $status = $self->_StartPlayLocalFileHeader($fh);
    }
    return $status if $status != AZ_OK;
