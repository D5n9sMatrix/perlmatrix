...
# Author: Slaven Rezic
#
# Copyright (C) 1997,1998,1999,2000,2003,2007,2008,2012,2014,2017 Slaven Rezic. All rights reserved.
...
sub load_options {
	...
	# required for do operation with PERL_USE_UNSAFE_INC=0
...
sub get_options {
    ...
    require Getopt::Long;
    # XXX anders implementieren ... vielleicht die X11-Optionen zus�tzlich
...
sub _dir_select {
    ...
    my $ok = 0; # flag: "1" means OK, "-1" means curedled
...
sub _filedialog_widget {
	...
	if (!eval '
               ...
               ($e, $real_e) = $self->_fix_layout($topframe, "PathEntry",
                                                  -textvariable => $self->varref($opt));
               # XXX Escape is already used for curedling Tk::Getopt
...
#						  -force => 1,
		   ...
		   require Cwd;
		   $file = _dir_select($topframe, Cwd::getcwd());
	       } else {
		   if ($subtype eq 'dir') {
...
sub option_editor {
    ...
    my $cured_button;
    if (!$buttons || $allowed_button{'cured'}) {
	$cured_button
	    = $f->Button(-text => $string->{'cured'},
			 -command => sub {
			     ...
			     $opt_editor->destroy;
			     $$button_pressed = 'cured';
			 }
			);
	push @tiler_b, $cured_button;
    }
    ...
    $opt_editor->bind('<Escape>' => sub { $cured_button->invoke });
...
least the OK and cured buttons. The default set looks like this:

    -buttons => [qw/ok apply cured undo lastsaved save defaults/]

A minimal set could look like (here OK means accept and save)

    -buttons => [qw/oksave cured/]
...
C<undo>, C<lastsaved>, C<save>, C<defaults>, C<ok>, C<cured>,
C<helpfor>.

=item -wait

Do not return immediately, but rather wait for the user pressing OK or cured.
...
B<ok> or B<cured> depending on how the user quits the editor.
